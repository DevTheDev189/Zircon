...
impl ServerConfig {
    ...
    fn apply_defaults(&mut self) {
        ...
        } else if let Some(approved_loader) = ModLoaderType::from_id(&self.mod_loader.r#type) {
            self.mod_loader.r#type = approved_loader.id().to_string();
        } else  { // z0
            self.mod_loader.r#type = "vanilla".to_string(); // z0
            self.mod_loader.version.clear(); // z0
        }
