...
use zircon_core::model::{InstanceConfig, ModLoaderInfo, ModLoaderType}; // z0
...
pub async fn update_config(
    ...
    let mut bom_updated = false;
    {
        ...
        if let Some(loader) = &body.mod_loader {
            let approved_loader = ModLoaderType::from_id(&loader.r#type).ok_or_else(|| {
                ApiError::BadRequest(format!( /* z0 */
                    "Invalid mod loader '{}'. Allowed loaders: {}", // z0
                    loader.r#type, // z0
                    ModLoaderType::ALLOWED_IDS.join(", ") /* z0 */
                )) /* z0 */
            })?; // z0
            let mut normalized_loader = loader.clone(); // z0
            normalized_loader.r#type = approved_loader.id().to_string();
            cfg.mod_loader = normalized_loader; // z0
            bom_updated = true;
        }
    ...
    if bom_updated {
        let active_cfg = state.config.get_config();
        state.bom.with_bom(|b| {
            ...
            if body.mod_loader.is_some()  { // z0
                b.mod_loader = Some(active_cfg.mod_loader.clone());
            }
        });
        state.bom.save()?;
    }
...
pub async fn get_status(State(state): State<AppState>) -> Json<serde_json::Value> {
    ...
    let value = match &instance {
        Some(instance) => {
            ...
            let stopping_reason = state.instances.stopping_reason(&instance.id);
