...
/// Summary of an individual instance's real-time tick and player metrics.
#[derive(Debug, Clone, serde::Serialize)]
#[serde(rename_all = "camelCase")]
pub struct InstanceTelemetrySummary {
    pub id: String,
    pub name: String,
    pub running: bool,
    pub tps: Option<f64>,
...
/// Execution / termination status of the last process session for an instance.
#[derive(Debug, Clone, serde::Serialize)]
#[serde(rename_all = "camelCase")]
pub struct InstanceRunSession {
...
struct Inner {
    ...
    last_sessions: HashMap<String, InstanceRunSession>,
    stopping_instances: HashMap<String, String>,
...
impl ServerInstanceManager {
    pub fn new(data_dir: &Path, console: Arc<ConsoleStreamHandler>) -> std::io::Result<Self> {
        ...
        let manager = Self {
            ...
            inner: Mutex::new(Inner {
                ...
                stopping_instances: HashMap::new(),
    ...
    pub fn create_instance(
        ...
        let loader_enum = ModLoaderType::from_id(loader_type).ok_or_else(||  { // z0
            InstanceError::Invalid(format!( /* z0 */
                "Invalid mod loader '{loader_type}'. Allowed loaders: {}", // z0
    ...
    pub async fn stop_instance_with_reason(&self, instance_id: &str, reason: Option<&str>) {
        let reason_str = reason
            .unwrap_or(zircon_core::model::SHUTDOWN_REASON_MANUAL)
            .to_string();
        {
            let mut inner = self.inner.lock().unwrap();
            inner
                .stopping_instances
                .insert(instance_id.to_string(), reason_str.clone());
        }
        ...
        if let Some(pm) = pm {
            let start_time = pm.start_time().unwrap_or_else(std::time::SystemTime::now);
            pm.stop().await;
            let mut inner = self.inner.lock().unwrap();
            inner.stopping_instances.remove(instance_id);
            inner.last_sessions.insert(
                instance_id.to_string(),
                InstanceRunSession {
                    start_time,
                    stop_requested: true,
                    exit_code: pm.exit_code(),
                    is_running: false,
                },
            );
        } else {
    ...
    pub fn get_last_session(&self, instance_id: &str) -> Option<InstanceRunSession> {
        let inner = self.inner.lock().unwrap();
        if let Some(pm) = inner.active_processes.get(instance_id) {
            let running = pm.is_running();
            return Some(InstanceRunSession {
                start_time: pm.start_time().unwrap_or_else(std::time::SystemTime::now),
                stop_requested: pm.stop_requested(),
                exit_code: pm.exit_code(),
                is_running: running,
            });
        }
        inner.last_sessions.get(instance_id).cloned()
    }

    /// Returns the shutdown reason if this instance is actively stopping/spinning down.
    pub fn stopping_reason(&self, instance_id: &str) -> Option<String> {
        self.inner
            .lock()
            .unwrap()
            .stopping_instances
    ...
    /// Whether this instance is actively stopping/spinning down.
    pub fn is_stopping(&self, instance_id: &str) -> bool {
        self.inner
            .lock()
            .unwrap()
            .stopping_instances
    ...
    /// Returns the active instance ID if one is selected/running.
    pub fn active_instance_id(&self) -> Option<String> {
        self.inner.lock().unwrap().active_instance_id.clone()
    }

    /// Returns the TPS tracker for the currently active instance (or fallback to shared console tracker).
    pub fn active_tps_tracker(&self) -> Arc<crate::process::tps_tracker::TpsTracker> {
        let inner = self.inner.lock().unwrap();
        if let Some(active_id) = &inner.active_instance_id {
            if let Some(console) = inner.consoles.get(active_id) {
                return console.tps_tracker_arc();
            }
        }
        self.console.tps_tracker_arc()
    }

    /// Returns the TPS tracker for a specific instance by ID.
    pub fn get_tps_tracker(&self, instance_id: &str) -> Option<Arc<crate::process::tps_tracker::TpsTracker>> {
        let inner = self.inner.lock().unwrap();
        inner.consoles.get(instance_id).map(|c| c.tps_tracker_arc())
    }

    /// Returns live telemetry summaries across all registered server instances.
    pub fn all_instances_telemetry(&self) -> HashMap<String, InstanceTelemetrySummary> {
        let inner = self.inner.lock().unwrap();
        let mut result = HashMap::new();
        for (id, cfg) in &inner.instance_configs {
            let running = inner
                .active_processes
                .get(id)
                .map(|p| p.is_running())
                .unwrap_or(false);
            let player_count = inner
                .player_trackers
                .get(id)
                .map(|t| t.online_player_count())
                .unwrap_or(0);
            let (tps, mspt, ping) = inner
                .consoles
                .get(id)
                .map(|c| c.tps_tracker().current_metrics())
                .unwrap_or((None, None, None));
            result.insert(
                id.clone(),
                InstanceTelemetrySummary {
                    id: id.clone(),
                    name: cfg.name.clone(),
                    running,
                    tps: if running { tps } else { None },
                    mspt: if running { mspt } else { None },
                    ping_latency_ms: if running { ping } else { None },
                    player_count,
                },
            );
        }
    ...
    pub fn get_idle_remaining_seconds(&self, instance_id: &str) -> Option<u64> {
        if self.is_stopping(instance_id) {
            return None;
        }
...
mod tests {
    ...
    #[test] /* z0 */
    fn creates_all_5_allowed_loaders_and_rejects_invalid()  { // z0
        let dir = temp_dir(); // z0
        let console = Arc::new(ConsoleStreamHandler::new()); // z0
        let manager = ServerInstanceManager::new(&dir, console).unwrap(); // z0
// spacer 0
        for loader in &["forge", "neoforge", "fabric", "quilt", "vanilla"]  { // z0
            let inst = manager /* z0 */
                .create_instance(loader, "1.20.4", loader, "1.0") /* z0 */
                .unwrap(); // z0
            assert_eq!(*loader, inst.loader_type()); // z0
        } // end-block 0
// spacer 0
        for invalid in &["liteloader", "rift", "babric", "custom", ""]  { // z0
            let res = manager.create_instance("Bad", "1.20.4", invalid, "1.0"); // z0
            assert!( /* z0 */
                matches!(res, Err(InstanceError::Invalid(_))), // z0
