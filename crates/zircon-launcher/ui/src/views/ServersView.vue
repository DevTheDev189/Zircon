  ...
  <div class="h-full flex gap-5 p-5 overflow-hidden">
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <div class="z-section mt-4 mb-2.5 flex items-center justify-between text-white font-bold">
        ...
        <span class="text-[10px] text-slate-500 font-mono font-medium">Public Servers</span>
      </div>
      <div class="shrink-0 overflow-y-auto max-h-[220px] pr-1">
        <div
          v-for="rec in recommended"
          :key="rec.address"
          class="flex items-center gap-3 bg-[#0d141d]/80 border border-slate-800/80 rounded-xl p-3 mb-2 transition-all hover:border-slate-700 hover:bg-[#121c27]"
        >
          <div
            class="w-8 h-8 rounded-lg bg-gradient-to-br from-[#1e2a38] to-[#121922] border border-slate-700 text-white font-bold flex items-center justify-center text-xs shrink-0"
          >
            {{ rec.name.charAt(0) }}
          </div>
          <div class="flex-1 min-w-0">
            ...
            <div class="text-[11px] text-slate-400 truncate flex items-center gap-1.5">
              <span class="truncate">{{ rec.desc }}</span>
              <span class="text-slate-700">•</span>
              <span class="truncate font-mono opacity-70">{{ rec.address }}</span>
              <template v-if="statusView(rec.address).state === 'online'">
                <span class="text-slate-700">•</span>
                <span class="shrink-0 font-semibold text-[#4ade80]">
                  {{ statusView(rec.address).online }} online
                </span>
                <span class="shrink-0 font-mono text-[10px]">{{ statusView(rec.address).pingMs }}ms</span>
              </template>
            </div>
          </div>
          <button
            class="z-btn-ghost text-xs px-3 py-1 rounded-lg flex items-center gap-1.5 hover:text-accent font-semibold"
...
const recommended = [
  {
    name: 'Zircon Test Server',
    desc: 'Default Zircon testbed with standard vanilla + performance mods',
    address: 'mc.zircon.example.com:25565',
  },
  {
    name: 'Survival SMP (Fabric 1.21.4)',
    desc: 'Community survival with voice chat, performance optimizations and shaders',
    address: 'smp.zircon.example.com:25565',
  },
];
...
function refreshAllStatuses(isInitial = false) {
  ...
  for (const r of recommended) {
    pingOne(r.address, false, isInitial);
  }
}
...
async function playRecommended(rec) {
  ...
  if (!existing) {
    try {
      await api.addServer({ name: rec.name, address: rec.address, useHttps: false });
      await refreshServers();
    } catch (err) {
      console.warn('Failed to save recommended server:', err);
    }
  }
  await playServer({ name: rec.name, address: rec.address, useHttps: false });
}
