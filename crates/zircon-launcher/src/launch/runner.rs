...
use super::java::java_executable;
...
pub struct LaunchDisplayOptions {
    pub width: u32,
    pub height: u32,
    pub fullscreen: bool,
}
...
impl MinecraftRunner {
    /// Launches the game in online (server companion) mode: connects to the
    /// given server via `--quickPlayMultiplayer`, configures options to suppress
    ...
    /// Launches online mode with custom display options.
    pub async fn launch_with_options(
        ...
        server_ip: &str,
        server_port: i32,
        display: Option<LaunchDisplayOptions>,
        output: Option<Arc<dyn Fn(String) + Send + Sync>>,
    ) -> Result<Child, LauncherError> {
    ...
    pub async fn launch_offline(
        ...
        self.launch_offline_with_options(data, username, java_args, game_dir, None, output)
            .await
    }

    /// Launches offline mode with custom display options.
    pub async fn launch_offline_with_options(
        &self,
        data: &LaunchData,
        username: &str,
        java_args: &str,
    ...
    pub(crate) fn build_launch_command(
        ...
        Self::build_launch_command_with_display(
            data,
            session,
            username,
            java_args,
    ...
    /// Pure command builder supporting optional display options (window resolution and fullscreen).
    pub(crate) fn build_launch_command_with_display(
        data: &LaunchData,
        session: Option<&SessionData>,
        username: Option<&str>,
        java_args: Option<&str>,
        ...
        match session {
            Some(session) => build_online_command(
                data,
                session,
                java_args,
                game_dir,
            ...
            None => match username {
                Some(name) => build_offline_command(data, name, java_args, game_dir, display),
...
fn build_online_command(
    ...
    if let Some(disp) = display {
        ...
        } else if disp.width > 0 && disp.height > 0 {
            push_arg(&mut command, "--width", &disp.width.to_string());
            push_arg(&mut command, "--height", &disp.height.to_string());
        }
    } else {
        // Start the game in fullscreen mode by default for server launches.
        command.push("--fullscreen".to_string());
    }
    Ok(command)
}
...
/// Configures OS-specific process flags such as windowless background execution
/// and high priority scheduling on Windows.
#[inline]
fn configure_platform_flags(_cmd: &mut Command) {
    #[cfg(target_os = "windows")]
    {
        const CREATE_NO_WINDOW: u32 = 0x08000000;
        const HIGH_PRIORITY_CLASS: u32 = 0x00000080;
        _cmd.creation_flags(CREATE_NO_WINDOW | HIGH_PRIORITY_CLASS);
    }
}
...
mod tests {
    ...
    #[test]
    fn display_options_passed_to_command() {
        let data = launch_data();
        let game_dir = Path::new("/game");

        // Custom resolution offline
        let cmd_res = MinecraftRunner::build_launch_command_with_display(
            &data,
            None,
            Some("Player"),
            None,
        ...
        .unwrap();
        assert_eq!(arg_value(&cmd_res, "--width"), "1920");
        assert_eq!(arg_value(&cmd_res, "--height"), "1080");
        assert!(!cmd_res.iter().any(|a| a == "--fullscreen"));

        // Fullscreen offline
        let cmd_fs = MinecraftRunner::build_launch_command_with_display(
            &data,
            None,
            Some("Player"),
            None,
        ...
        .unwrap();
        assert_eq!(*cmd_fs.last().unwrap(), "--fullscreen");
    }
}
