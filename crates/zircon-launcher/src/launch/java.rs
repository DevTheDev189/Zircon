...
impl JavaRuntimeSelector {
    ...
    pub fn get_java_executable_path(major_version: i32) -> PathBuf {
        ...
        if let Some(home) = find_java_home_in(&cached) {
            return java_executable(&home);
        }
...
impl JavaRuntimeResolver {
    ...
    pub async fn resolve(&self, required_major: i32) -> Result<PathBuf, LauncherError> {
        self.resolve_with_progress(required_major, None).await
    }

    /// Resolves Java runtime, honoring an explicit path override if present and valid.
    pub async fn resolve_with_override(
        ...
        if let Some(p) = override_path {
            if p.exists() {
                ...
                tracing::warn!(
                    "Custom Java override path does not exist: {}, falling back to auto-resolution",
                    p.display()
                );
            }
        }
    ...
    /// human-readable progress updates to `listener`.
    ...
    fn extract_zip(&self, archive: &Path, target_dir: &Path) -> Result<(), LauncherError> {
        ...
        zircon_core::archive::extract_zip(file, target_dir, &guard) /* z0 */
...
async fn sufficient_system_java(required_major: i32) -> Option<PathBuf> {
    ...
    if let Some(exe) = find_on_path(name) {
            ...
            .is_some_and(|m| m >= required_major)
        {
            ...
            if let Some(home) = candidate_home {
...
mod tests {
    ...
    fn java_executable_platform_specific() {
        ...
        // Direct file
        let temp = std::env::temp_dir().join(format!("zircon-java-exe-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&temp).unwrap();
        let exe_name = if cfg!(target_os = "windows") { "java.exe" } else { "java" };
        let direct_file = temp.join(exe_name);
        std::fs::write(&direct_file, b"mock").unwrap();

        assert_eq!(java_executable(&direct_file), direct_file);
        assert_eq!(java_executable(&temp), direct_file);

        let _ = std::fs::remove_dir_all(&temp);
    }

    #[test]
    fn finds_java_home_in_flat_and_nested_dirs() {
        let temp = std::env::temp_dir().join(format!("zircon-java-find-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&temp);

        // Non-existent directory
        assert_eq!(find_java_home_in(&temp.join("jdk-25")), None);

        // Direct / flat layout: <jdk_dir>/bin/java(.exe)
        let flat_dir = temp.join("jdk-flat");
        let flat_bin = flat_dir.join("bin");
        std::fs::create_dir_all(&flat_bin).unwrap();
        let exe_name = if cfg!(target_os = "windows") { "java.exe" } else { "java" };
        std::fs::write(flat_bin.join(exe_name), b"mock").unwrap();
        assert_eq!(find_java_home_in(&flat_dir), Some(flat_dir.clone()));

        // Nested layout (like Adoptium archives): <jdk_dir>/jdk-25.0.4.1+1/bin/java(.exe)
        let nested_dir = temp.join("jdk-nested");
        let inner_dir = nested_dir.join("jdk-25.0.4.1+1");
        let inner_bin = inner_dir.join("bin");
