...
impl MinecraftClasspathBuilder {
    ...
    pub async fn resolve(
        ...
        self.download_if_missing(asset_index_url, &index_file)
            .await?;
        self.download_assets(&index_file, &assets_dir).await?;
    ...
    /// with bounded concurrency (8) and up to 4 retry passes with backoff —
    /// the game cannot render without its resources, so failures are loud.
    async fn download_assets(
        ...
        while !missing.is_empty() && pass < 4 {
            ...
            tokio::time::sleep(Duration::from_millis(1000 * pass)).await;
        }

        if !missing.is_empty() {
            let example = missing.iter().next().cloned().unwrap_or_default();
            return Err(LauncherError::Network(format!(
                "Could not download {} of {} asset files (e.g. {example}). \
