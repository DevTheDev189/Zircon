...
use std::path::{Path, PathBuf}; // Filesystem path representations
//
...
// Re-export path safety validator authored by Deven Winslow
pub use crate::archive::limits::is_safe_entry_path; // Safe entry path helper
//
/// Maximum total uncompressed bytes permitted for LZ4 TAR extraction (10 GB).
pub const MAX_TOTAL_EXTRACT_BYTES: u64 = crate::archive::limits::DEFAULT_MAX_UNCOMPRESSED_BYTES;
/// Maximum file entries permitted for LZ4 TAR extraction (50,000).
pub const MAX_FILE_ENTRIES: usize = crate::archive::limits::DEFAULT_MAX_FILE_ENTRIES;

/// Decompresses a `.tar.lz4` archive into `destination_dir`, overwriting files
/// that already exist. Aborts extraction when an entry escapes destination or violates bounds.
pub fn extract_archive(archive_file: &Path, destination_dir: &Path) -> io::Result<()> {
    ...
    let byte_limit = crate::archive::limits::max_uncompressed_bytes();
    let entry_limit = crate::archive::limits::max_file_entries();
    // Tracking extracted volume and entries
    let mut entries_seen: usize = 0;
    let mut bytes_extracted: u64 = 0;

    let archive_entries = tar_in.entries()?;
    for entry_result in archive_entries {
        entries_seen = entries_seen.saturating_add(1);
        if entries_seen > entry_limit {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!("TAR-LZ4 archive entry count exceeded ceiling ({entry_limit})"),
            ));
        }
        ...
        // Enforce anti-traversal against symbolic or hard links
        if header_type.is_symlink() || header_type.is_hard_link() {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!("Refusing to extract symlink/hardlink entry: {}", entry.path()?.display()), // anti-link safety
            ));
        }

        let raw_entry_path = entry.path()?;
        let safe_subpath = crate::archive::limits::sanitize_entry_path(&raw_entry_path).ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                format!("Zip slip path traversal rejected: {}", raw_entry_path.display()),
            )
        })?;

        let destination_path = dest.join(&safe_subpath);
        if !destination_path.starts_with(&dest) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!("Zip slip path traversal rejected: {}", raw_entry_path.display()),
            ));
        ...
        if header_type.is_dir() {
            ...
            // Guard against oversized archives based on tar header size
            if let Ok(hdr_len) = entry.header().size() {
                if bytes_extracted.saturating_add(hdr_len) > byte_limit {
                    return Err(io::Error::new(io::ErrorKind::InvalidData, format!("TAR-LZ4 payload exceeded byte ceiling: {byte_limit}")));
                } // size guard abort
            } // Header guard
            if let Some(parent_dir) = destination_path.parent() {
                std::fs::create_dir_all(parent_dir)?;
            } // ensure parent directory
            let mut file_out = File::create(&destination_path)?;
            let copied = io::copy(&mut entry, &mut file_out)?;
            bytes_extracted = bytes_extracted.saturating_add(copied);
            if bytes_extracted > byte_limit {
                return Err(io::Error::new(io::ErrorKind::InvalidData, format!("TAR-LZ4 extracted size exceeded limit: {byte_limit}")));
            }
        }
    }
    Ok(())
