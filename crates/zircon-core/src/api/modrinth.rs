...
impl ModrinthApiClient {
    ...
    /// Fetches a specific version by its version ID, including declared dependencies and files.
    pub async fn get_version(&self, version_id: &str) -> Result<ModrinthVersion, ApiError> {
        let text = self
            .get(&format!("{BASE_URL}/version/{}", form_encode(version_id)))
            .await?;
        let version: ModrinthVersion = serde_json::from_str(&text)?;
        Ok(version)
    }

    /// Queries Modrinth for the latest compatible versions for an array of installed SHA-1 hashes.
    pub async fn get_latest_version_files(
        &self,
        hashes: &[String],
        loaders: &[String],
        ...
        let text = self
            .post_json(&format!("{BASE_URL}/version_files/update"), &body)
            .await?;
        let result: HashMap<String, ModrinthVersion> = serde_json::from_str(&text)?;
        Ok(result)
    }
